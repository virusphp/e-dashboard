...
class RawatInap extends Koneksi
{
    public function getData($request)
    {
        ...
        $data = DB::connection($this->conn)->table('Registrasi AS reg')
            ...
            ->join('Rawat_Inap AS ri', function($join) {
                $join->on('reg.no_reg','=','ri.no_reg')
                ->join('Pegawai AS pg', 'ri.kd_dokter','=','pg.kd_pegawai');
            })
            ->join('Pasien AS p', function($join) {
                $join->on('reg.no_RM','=','p.no_RM');
            })
            ->select('reg.no_reg','reg.kd_cara_bayar','ri.no_RM','ri.kd_tempat_tidur','pg.nama_pegawai','p.nama_pasien','p.jns_kel')
